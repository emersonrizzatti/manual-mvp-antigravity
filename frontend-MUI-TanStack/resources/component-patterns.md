...
### Mobile-First Data Density (DataGrid vs Cards)

When rendering arrays of complex data, adapt the UI based on device width to avoid horizontal scrolling:

```typescript
import { useMediaQuery, useTheme } from '@mui/material';

export const ResponsiveDataList: React.FC<{ data: MyData[] }> = ({ data }) => {
    ...
    // On Desktop, an expansive DataGrid or Table is preferred
    return <DataGrid rows={data} columns={myDesktopColumns} />;
};
```

### Cache-First For Unstable Networks

Configure `useSuspenseQuery` with elevated `staleTime` and offline considerations to handle spotty internet contexts (like warehouses or the field).
