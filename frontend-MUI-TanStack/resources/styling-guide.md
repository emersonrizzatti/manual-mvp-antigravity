...
Use the `display` property within `sx` to show/hide elements specifically across breakpoints. Crucial for data-dense tables on mobile vs desktop.
...
// Or for TableCells to hide columns on mobile:
...
All interactive elements **MUST** have a minimum height of `44px` or `48px` to ensure touch accessibility on mobile devices.

```typescript
<Button
    variant="contained"
    sx={{ 
        minHeight: 48, // 👈 Required for touch targets
        width: { xs: '100%', md: 'auto' } // Full width on mobile often provides better UX
    }}
>
    Confirm Action
</Button>
